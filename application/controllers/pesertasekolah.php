...
class Pesertasekolah extends CI_Controller {
	...
	public function index()
	{
        ...
        if ($this->session->userdata('namapesertasekolah')) {            
            $this->session->unset_userdata('namapesertasekolah');
        }
		date_default_timezone_set('Asia/Jakarta');
		$data['tanggal']=date("Y-m-d");  

        $config['base_url']=base_url()."pesertasekolah/index";
        $config['total_rows']= $this->db->query("SELECT `id_peserta` FROM `peserta` JOIN `pendidikan` WHERE `peserta`.`id_lembaga`=`pendidikan`.`id_lembaga` AND `jenis`=0 ;")->num_rows();
        $config['per_page']=10;
        $config['num_links']=3;
        ...
        $data['datapeserta']=$this->m_peserta->getpesertasekolah($config); 

        $data['title']='Peserta dari Sekolah';
        $data['page']='v_pesertasekolah';
    ...
    public function formtambah()
    {
        date_default_timezone_set('Asia/Jakarta');
        $data['tanggal']=date("Y-m-d"); 
        $data['lembaga']=$this->m_wilayah->get_all_lembagasekolah();
        $data['divisi']=$this->m_wilayah->get_all_divisi();                
        $data['path'] = base_url('assets');

        $data['title']='Tambah Peserta dari Sekolah';
        $data['page']='v_tambahpesertasekolah';
        $this->load->view('v_dashboard', $data);
    }

    function tambah(){
        $this->form_validation->set_rules('nim','Nomor Induk','required|trim|min_length[4]|max_length[50]');        
    ...
    public function cari()
    {
        ...
        if(isset($_POST['submit'])){       
            ...
            {   
                ...
                $namapesertasekolah=$this->input->post('nama');
                $this->session->set_userdata("namapesertasekolah", $this->input->post('nama'));
            }
              
        }else{                   
            $namapesertasekolah=$this->session->userdata('namapesertasekolah');
        }

        $config['base_url']=base_url()."pesertasekolah/cari/index";
        $config['total_rows']= $this->db->query("SELECT `id_peserta`, `peserta`.`nama` AS `nama_peserta`, `peserta`.`id_lembaga`, `peserta`.`id_divisi`, `divisi`.`nama` AS `nama_divisi`, `pendidikan`.`nama` AS `nama_pendidikan` FROM `peserta` LEFT JOIN `divisi` ON `peserta`.`id_divisi` = `divisi`.`id_divisi` LEFT JOIN `pendidikan` ON `peserta`.`id_lembaga` = `pendidikan`.`id_lembaga` WHERE `jenis` = 0 AND `peserta`.`nama` LIKE '%".$namapesertasekolah."%';")->num_rows();
        $config['per_page']=10;
        $config['num_links']=3;
        ...
        $data['datapeserta']=$this->m_peserta->getcari_pesertasekolah($namapesertasekolah, $config); 
    ...
    function ubahpeserta(){
        ...
        if($this->form_validation->run()==false){
            $this->session->set_flashdata('warning',validation_errors());
            redirect("pesertasekolah/detail/$id");
        }else{
